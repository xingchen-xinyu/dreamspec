...
检查主插件和依赖插件是否有新版本，由用户决定是否升级。各插件具体升级命令见 [PLUGINS.md](../../reference/PLUGINS.md)。

**A. 主插件（dreamspec）版本检测：**

1. 更新 marketplace：`claude plugin marketplace update dreamspec-market`（升级命令详见 [PLUGINS.md](../../reference/PLUGINS.md) DreamSpec 条目）
2. 读取 `.claude/plugin-state.json` 中的 `plugin_version`（全新项目无此字段，视为首次安装）
3. 获取远端最新版本（不可读本地缓存，本地缓存可能未同步）：
   - 读取当前项目安装的 `plugin.json`，从中提取 `repository` 字段获取 GitHub 仓库地址（如 `https://github.com/xingchen-xinyu/dreamspec`）
...
- 用户选择升级 → 从 [PLUGINS.md](../../reference/PLUGINS.md) 获取各插件的「升级命令」并依次执行
- 用户选择跳过 → 保持当前版本，继续下一步
...
**下一步：** /ds:vision（制定产品愿景，完成后自动回填项目信息），待 vision 完成后再 /ds:build（开始版本交付）
```
