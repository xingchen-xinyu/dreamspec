...
> 完整走全部 6 步：依赖检测（含版本）→ marketplace 刷新 → 确认 → 安装与升级 → 合规复检 → 汇报。
...
**依赖插件：**
1. openspec：
   - 如阶段一已执行设备级安装 → `openspec update`（跳过 npm install 和 config profile，阶段一已完成）
   - 如阶段一仅执行项目级配置 → `npm install -g @fission-ai/openspec@latest` → `openspec update`（跳过 config profile，阶段一已完成）
...
检查项：
  ...
  - plugin-state.json 结构是否与最新模板一致（plugin_version 已在 Step A4 自动同步，无需再检查）
...
合规复检内容同模式 A Step A5，检查 DreamSpec 升级后项目结构是否需要同步（plugin_version 已在 Step B2 自动同步，无需再检查）。
