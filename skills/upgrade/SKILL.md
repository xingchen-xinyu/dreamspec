...
3. 当前安装版本：使用前置条件中 `claude plugins list` 输出的 dreamspec 版本号。如果项目已初始化且 `.claude/plugin-state.json` 中存在 `plugin_version` 字段，以 state.json 中的记录为准
4. 获取远端最新版本（不可读本地缓存，本地缓存可能未同步）：
   - 从 marketplace 信息或 GitHub Release API 获取 DreamSpec 远端最新版本号（仓库地址：`https://github.com/xingchen-xinyu/dreamspec`）
   - 或直接 fetch `https://raw.githubusercontent.com/xingchen-xinyu/dreamspec/main/.claude-plugin/plugin.json` 读取 `version` 字段
5. 对比：
...
- 升级前版本检测以 `claude plugins list` 输出为准，`plugin-state.json` 为辅助记录
